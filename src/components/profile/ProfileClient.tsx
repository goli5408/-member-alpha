...
export default function ProfileClient({
  ...
  const [vibe, setVibe] = useState<{ emoji: string; label: string } | null>(
    profile?.vibe_emoji ? { emoji: profile.vibe_emoji, label: profile.vibe_label ?? "" } : null
  );
  ...
  // Hidden inputs mirror state so the server action can read them
  ...
  function handleHeroChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    setHeroSrc(URL.createObjectURL(file));
  }
  ...
  const effectivePronouns = pronouns === "custom" ? customPronouns : pronouns;

  return (
    <>
      <form action={formAction}>
        {/* Hidden fields that mirror client state */}
        <input type="hidden" name="display_name" value={displayName} />
        <input type="hidden" name="bio" value={bio} />
        <input type="hidden" name="pronouns" value={effectivePronouns} />
        <input type="hidden" name="hometown" value={hometown} />
        <input type="hidden" name="vibe_emoji" value={vibe?.emoji ?? ""} />
        <input type="hidden" name="vibe_label" value={vibe?.label ?? ""} />
        ...
        <div className="pb-10">
          ...
          <div className="px-5 -mt-10 flex items-end gap-4 mb-5">
            ...
            <div className="pb-1 flex-1 min-w-0">
              ...
              <p className="text-xs mt-0.5" style={{ color: "rgba(65,70,81,0.60)" }}>{effectivePronouns}</p>
            </div>
          </div>
          ...
          <div className="px-4">
            ...
            <button
              ...
              className="w-full mt-4 rounded-2xl py-3.5 text-sm font-bold flex items-center justify-center gap-2 transition active:scale-[0.98] disabled:opacity-60"
              style={
